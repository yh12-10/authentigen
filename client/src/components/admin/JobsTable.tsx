            ...
            {jobsQuery.data?.map((j) => (
              <TableRow key={j.id}>
                ...
                <TableCell className="text-sm">
                  <div>{j.userName ?? "—"}</div>
                  <div className="text-xs text-muted-foreground">{j.userEmail}</div>
                </TableCell>
                ...
                <TableCell><Badge variant="secondary">{j.intensity}</Badge></TableCell>
                <TableCell className="text-right font-mono">{j.creditsUsed}</TableCell>
                <TableCell className="max-w-[180px] truncate text-sm" title={j.originalFilename}>{j.originalFilename}</TableCell>
