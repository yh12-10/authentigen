...
const AVATAR_COLORS = ["#F5A623", "#4F8EF7", "#10B981", "#A855F7", "#EC4899", "#FB923C"];

export function Testimonials() {
  return (
    <Carousel
      opts={{ align: "start", loop: true }}
      plugins={[Autoplay({ delay: 5000, stopOnInteraction: true }) as unknown as never]}
      className="w-full"
    >
      <CarouselContent className="-ml-4">
        {TESTIMONIALS.map((t, i) => (
          <CarouselItem key={t.name} className="pl-4 sm:basis-2/3 md:basis-1/2 lg:basis-1/3">
              ...
              <CardContent className="pt-6 pb-5 relative">
                <div className="flex gap-0.5 mb-3">
                  {Array.from({ length: t.rating }).map((_, j) => (
                    <Star key={j} className="size-4 fill-[#F5A623] text-[#F5A623]" />
                  ))}
                </div>
