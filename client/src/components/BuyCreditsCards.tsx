...
const PACK_META: Record<
  string,
  { icon: any; gradient: string; popular?: boolean }
> = {
    ...
    return (
      <Card className="glass border-amber-500/20">
        <CardHeader>
          ...
          <CardDescription>
            Credit purchases are temporarily unavailable. Set{" "}
            <code className="text-xs">STRIPE_SECRET_KEY</code>,{" "}
            <code className="text-xs">STRIPE_WEBHOOK_SECRET</code>, and the
            three Stripe price IDs in your <code className="text-xs">.env</code>{" "}
            to enable checkout.
          </CardDescription>
        </CardHeader>
      </Card>
        ...
        return (
          ...
          >
            ...
            <CardHeader>
              <div
                className={cn(
                  "inline-flex size-10 items-center justify-center rounded-md bg-gradient-to-br",
                  meta?.gradient
                )}
              >
