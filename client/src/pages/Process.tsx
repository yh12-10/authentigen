...
import {
  Sparkles, ArrowLeft, Download, CheckCircle2,
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden" style={{ clipPath: `inset(0 0 0 ${sliderPos}%)` }}>
        <img src={processedUrl} alt="Humanized" className="absolute inset-0 w-full h-full object-cover" draggable={false} />
      </div>
      <div
        className="absolute top-0 bottom-0 w-px bg-[#F5A623]"
        style={{ left: `${sliderPos}%`, transform: "translateX(-50%)", boxShadow: "0 0 20px rgba(245,166,35,0.6)" }}
      >
      ...
      <div className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-semibold glass text-foreground">Original</div>
      <div className="absolute top-3 right-3 px-2.5 py-1 rounded-full text-xs font-semibold bg-[#F5A623] text-black">Humanized</div>
    </div>
  );
}

function SyncedVideoPair({ originalUrl, processedUrl, isVideo }: { originalUrl: string; processedUrl: string; isVideo: boolean }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      ...
      <div>
        <div className="text-xs text-muted-foreground mb-2 font-medium">Humanized</div>
        {isVideo ? (
          <video
            ref={bRef}
            src={processedUrl}
          ...
          <img src={processedUrl} alt="Humanized preview frame" className="w-full rounded-xl object-cover" style={{ aspectRatio: "16/9" }} />
        )}
      </div>
    </div>
  );
...
function ProgressBar({ progress }: { progress: number }) {
  return (
    <div className="w-full bg-secondary rounded-full h-2 overflow-hidden">
      <div className="h-full rounded-full progress-bar-animated transition-all duration-500" style={{ width: `${progress}%` }} />
    </div>
  );
}
        ...
        <div className="max-w-3xl mx-auto">
          <div className="glass rounded-2xl p-6 mb-6">
            {(job?.status === "pending" || job?.status === "processing" || !job) && (
              <div className="flex flex-col items-center text-center mb-6">
                ...
                <h1 className="font-serif text-2xl mt-4">
                  {job?.status === "pending" ? "Queued for Processing" : "Humanizing Your Content"}
                </h1>
                <p className="text-muted-foreground text-sm mt-1">{job?.originalFilename ?? "Loading…"}</p>
              </div>
            )}
            ...
            {job?.status === "completed" && (
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-sm">{job.originalFilename}</p>
                </div>
            ...
            {job && (job.status === "processing" || job.status === "pending") && (
              <>
                <div className="mb-4">
                  <StepIndicators progress={job.progress} status={job.status} />
                </div>
                <div className="flex justify-between text-xs text-muted-foreground mb-2">
                  <span>Progress</span>
                  <span>{job.progress}% • {getETA()}</span>
                </div>
            ...
            {job?.status === "completed" && (
              <div className="mt-4 flex flex-wrap gap-3">
                <Button onClick={handleDownload} className="glow-gold-sm relative overflow-hidden">
          ...
          {job?.status === "completed" && job.processedUrl && (
            <div className="glass rounded-2xl p-6">
              ...
              {job.type === "video" ? (
                ...
                <div>
                  <p className="text-muted-foreground text-sm mb-4">Drag the slider to compare original vs. humanized.</p>
                  <ComparisonSlider originalUrl={job.originalUrl} processedUrl={job.processedUrl} />
                </div>
              )}
            </div>
          )}
